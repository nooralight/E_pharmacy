...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
    ...
    <body>
      ...
      <div style="text-align: center;margin-top: 50px;">
        <button id="AddNew" onclick="window.location.href='{{route("addEmployee")}}'">Add Employee</button>
      </div>
      ...
      <table class="table table-dark table-hover table-bordered table-striped">
        <thead>
          <tr>
            ...
            <th style="padding: 18px" scope="col">Id no</th>
            <th style="padding: 18px" scope="col">phone</th>
            <th style="padding: 18px" scope="col">Email</th>
        ...
        <tbody>
            ...
            <th scope="row">{{$item->id}}</th>
              ...
              <td>{{$item->phone}}</td>
              <td>{{$item->email}}</td>
              <td style="text-align: center"><span><a href="{{url("/emp/".$item->id."/update")}}" style="color: #f1c40f">Update</a></span>
              <button style="margin-left: 20px" onclick="document.getElementById('emp-delete-{{$item->id}}').submit()" >Delete</button></td>
              <form style="display: none" method="post" action={{route("emp.delete", $item->id)}} id={{"emp-delete-".$item->id}}>
                @csrf
                @method("PUT")
            </form>
          </tr>
